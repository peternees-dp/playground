...
import { ChevronRight, Star, Bot, Download, BookOpen, Shield, CheckCircle2, Check, MessageSquare, Zap, ExternalLink } from 'lucide-react'
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2 flex-wrap">
                    ...
                    {app.aiPowered && !isAgenticAI && (
                      <span className={`px-2.5 py-1 text-xs font-medium rounded-full flex items-center gap-1 ${darkMode ? 'bg-purple-900/30 text-purple-400' : 'bg-purple-100 text-purple-700'}`}>
                        <Bot className="w-3 h-3" /> AI-Powered
                      </span>
                    )}
                    {app.isNew && (
                      <span className={`px-2.5 py-1 text-xs font-semibold rounded-full ${darkMode ? 'bg-green-900/30 text-green-400' : 'bg-green-100 text-green-700'}`}>
                        New
                      </span>
                    )}
                    {app.developerType === 'native' && (
                      <span className={`px-2.5 py-1 text-xs font-medium rounded-full ${darkMode ? 'bg-blue-900/30 text-blue-400' : 'bg-blue-100 text-blue-700'}`}>
                        Native Integration
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-4 mb-4 flex-wrap">
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`text-sm ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>({app.reviews?.toLocaleString()} reviews)</span>
                    </div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    ...
                    <button className={`px-6 py-3 rounded-xl font-semibold transition-colors flex items-center gap-2 ${darkMode ? 'bg-gray-800 text-white hover:bg-gray-700' : 'bg-gray-100 text-gray-900 hover:bg-gray-200'}`}>
            ...
            {hasDialpadEverywhere && (
              <div className={`rounded-2xl p-5 mb-6 flex items-center gap-4 ${darkMode ? 'bg-gradient-to-r from-indigo-900/50 to-purple-900/50 border border-indigo-700/30' : 'bg-gradient-to-r from-indigo-100 to-purple-100 border border-indigo-200'}`}>
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${darkMode ? 'bg-indigo-600' : 'bg-indigo-500'}`}>
                  <Zap className="w-6 h-6 text-white" />
                </div>
                <div className="flex-1">
                  <h3 className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>Dialpad Everywhere</h3>
                  <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                    Access live transcription and AI coaching directly within {app.name}
                  </p>
                </div>
